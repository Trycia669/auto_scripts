...
-1.1、 PRIVATE_REPO_TOKEN （用于读取私库的 token，可跟其他库共用）

-1.2、 HOHAI_BATCH （hohai的账号环境变量）

```
HOHAI_BATCH='username1,password1
username2,password2,tg_bot_token,tg_chat_id'
```
### 这上面的单引号你自己不要加上去哈
...
1、不发 TG：username,password

2、发 TG：username,password,tg_bot_token,tg_chat_id
...
## 3、修改定时任务执行时间。
### 去hohai-checkin.yml里面修改你的定时任务的执行时间
